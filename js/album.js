...
function getnext(){
    ...
    pics.pop();
    pics.unshift(give_up);//向数组开头添加元素并返回新长度
    ...
    dotmov();
    pics[len-2].onclick=function(){//鼠标点击处理事件
        getnext();
    }
...
function getpre(){
    ...
    pics.shift();
    for(var i=0;i<pics.length;i++){
        pics[i].style.zIndex=i;//图片变换
        pics[i].style.transform="scale(1)";//缩放
    }
    pics[len-2].style.left="0px";
    pics[len-2].style.opacity=0.5;//上一张不透明设置
    pics[len-3].style.opacity=0;
    pics[len-4].style.opacity=0;
    pics[len-1].style.zIndex=100;
    pics[len-1].style.left="200px";
    pics[len-1].style.transform="scale(1.3)";
    pics[len-1].style.opacity=1;//本张不透明设置
    pics[len].style.left="400px";
    pics[len].style.opacity=0.5;//下一张不透明设置
    dotmov();
